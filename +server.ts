...
async function google(query: string, baseUrl: string, description: string = "") {
  ...
  try {
    ...
    for (let i = 0; i < searchResults.length; i++) {
      ...
      const titleIgnores = ["ebay", "amazon", "REDACTED"]; // we don't want our results either.
      let skip = false;
      ...
      try {
        ...
        if (body.lastUsed !== priceFound) {
          await fetch(`${baseUrl}/api/db/getGoogleRegex`, {
            ...
            body: JSON.stringify({
              domain: item.domain,
              shop: extractDomain(item.domain),
              lastUsed: priceFound,
            }),
            headers: {
              "Content-Type": "application/json",
        ...
        if (item.price.includes("to")) {
          ...
          item.price = (item.price.split('to')[item.price.split('to').length - 1]).replaceAll(' ', '').replaceAll('£', '');
          clog("Changing to");
          clog(item.price);
        }
...
export const POST: RequestHandler = async ({ request, url }) => {
  ...
  console.log("google Array: ", Array.isArray(body), body);

  if (Array.isArray(body)) products = body;
  else
    products = await fetch(
      `${baseUrl}/api/db/products?orderby=googleLast&order=asc`,
    ).then((resp) => resp.json());

  const result: { REDACTED: any; price: any; shipping: any; href: any }[] = [];
      ...
      const batchPromises = batchProducts.map(
        async (product: {
          ...
          try {
            ...
            cerr(`Error processing product ${product.REDACTED}:`, error);
          }
        },
      );
